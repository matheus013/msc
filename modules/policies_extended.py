"""
policies_extended.py — 12 Políticas de Inventário para Comparativo Estendido
...
def _get_search_bounds(cfg: dict, demand: np.ndarray) -> tuple:
    ...
    Valores do config são mínimos; referências EOQ/SS ampliam quando necessário.
    """
...
def _eval_static(demand, cfg, ROP, Q, SS):
    ...
    # InventoryEnv.__init__ already calls reset(); use _state() to avoid double-reset
...
class SsPolicyClass:
    def __init__(self, demand, cfg):
        ...
        lt  = scfg.get("lead_time", 2)
        z   = cfg.get("HEURISTIC", {}).get("z_score", 1.645)
        self.s = mu * lt + z * std * np.sqrt(lt)          # ponto de reposição
        self.S = self.s + max(1.0, np.sqrt(                # nível alvo S
            2 * mu * len(demand) *
...
class GAPolicyOptimizer:
    ...
    def _setup(self):
        ...
        # Sigma proporcional ao maior range para boa exploração
