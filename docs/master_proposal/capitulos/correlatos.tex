...
A variante GA-PPO, que substitui o DQN por uma política contínua baseada em PPO~\citep{Vanvuchelen2020PPOJointReplenishment,Gijsbrechts2022DRLInventoryMSOM}, \textbf{também é proposta por} \citet{Zabraoui2025GaDQN}: o Algoritmo~1 daquele trabalho inicializa indistintamente um agente PPO ou DQN a partir da configuração $\theta^*$ obtida pelo GA, e os autores comparam as duas variantes, reportando nível de serviço de 94\% para GA-DQN contra 91\% para GA-PPO, com o GA-PPO exibindo padrões de pedido mais reativos e maior volatilidade do estoque. Nenhuma das duas arquiteturas híbridas é, portanto, contribuição original desta dissertação: ambas são adotadas como políticas especialistas do portfólio, com formulação herdada do artigo-base.

Registra-se que a maior volatilidade do GA-PPO observada por \citet{Zabraoui2025GaDQN} em regime de baixa intermitência (CV~$<1$) é consistente com a frequência de pedidos elevada verificada para a mesma arquitetura no Experimento~1 desta dissertação, sob regime \textit{Lumpy}. Trata-se de convergência de evidência entre bases e regimes distintos, e não de achado inédito. A presença de GA-DQN e GA-PPO no portfólio possibilita ao PSE identificar, para cada perfil operacional de demanda, se uma abordagem híbrida oferece melhor compromisso entre custo e nível de serviço ou se políticas clássicas permanecem mais adequadas naquele recorte.
