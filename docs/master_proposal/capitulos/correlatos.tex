...
O AIPE não propõe novos algoritmos de otimização ou de aprendizado: GA, SA, PSO, DE, DQN, PPO, SARSA e as arquiteturas híbridas GA-RL são tratados como componentes de um portfólio de políticas candidatas, cada um avaliado empiricamente sob o mesmo protocolo experimental, e não como contribuições algorítmicas desta dissertação. A contribuição reside no \textit{benchmark} empírico desse portfólio sobre dados reais de varejo, na comparação direta entre aplicar uma política única globalmente e selecionar políticas por perfil operacional, e no treinamento do \textit{Policy Selection Engine} a partir dessa evidência.
