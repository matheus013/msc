...
EOQ & lote econômico $Q^*$ ($Q_{\text{ref}}$ na implementação) & $\sqrt{2\hat{\mu} T\, c_o^{\text{fix}}/h}$ & $c_o^{\text{fix}}{=}50$, $h{=}1$ (globais); $\hat{\mu}$ estimado por série; $T$ é o horizonte de referência \\
EOQ & ponto de pedido $s$ ($s_{\text{ref}}$ na implementação) & $\hat{\mu} L + z\sigma\sqrt{L}$ & $L{=}2$; $z{\approx}0{,}97$; ponto de pedido também chamado de ponto de reposição \\[4pt]

\multicolumn{4}{@{}l}{\textit{$(s,S)$}} \\[2pt]
$(s,S)$ & ponto de pedido $s$ & $\hat{\mu} L + z\sigma\sqrt{L}$ (igual a $s_{\text{ref}}$ da EOQ) & $L{=}2$ \\
