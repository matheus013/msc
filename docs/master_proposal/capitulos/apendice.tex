...
Ambiente & lead time & 2 ciclos & uniforme; mesmo valor para todas as políticas \\
Ambiente & estoque inicial $I_0$ & escalonado por série & $\mathrm{rop\_ref} = \hat{\mu} L + z\hat{\sigma}\sqrt{L}$; ajustado por série \\
Ambiente & tipo de falta & perda de venda & demanda não atendida é perdida; sem acúmulo \\
Ambiente & regra de arredondamento & inteiro superior & aplicada a todos os pedidos calculados \\
Ambiente & frequência de reavaliação (AIPE) & 6 ciclos & intervalo $\Delta t$ definido conceitualmente \\[4pt]
...
Custos & custo fixo de pedido $c_o^{\text{fix}}$ & 50,0 & por ordem emitida \\
Custos & custo variável de pedido $c_o^{\text{var}}$ & 0,5 & por unidade pedida \\
...
Nas políticas clássicas, os parâmetros de reposição são calculados diretamente a partir das estatísticas históricas da série e dos parâmetros globais de custo. Nas meta-heurísticas, por sua vez, cada candidato é uma parametrização da política $(s,Q)$, em que $s$ representa o ponto de reposição e $Q$ representa a quantidade de pedido. A avaliação de cada candidato ocorre pela execução da política correspondente no ambiente de simulação, retornando os indicadores de custo total de inventário, nível de serviço, ruptura, efeito chicote e frequência de pedidos.

Para o Algoritmo Genético, cada indivíduo da população é codificado como um vetor contínuo $(s,Q)$. A população inicial é amostrada dentro dos limites operacionais admissíveis para a série avaliada. A seleção é realizada por torneio, favorecendo indivíduos com melhor avaliação; o cruzamento combina os parâmetros de dois indivíduos pais por meio de cruzamento do tipo \textit{Blend}; e a mutação aplica perturbação gaussiana aos genes, seguida de ajuste aos limites admissíveis. Dessa forma, o GA busca combinações de $(s,Q)$ que reduzam o custo simulado sem perder viabilidade de serviço.
...
EOQ & lote econômico $Q^*$ ($Q_{\text{ref}}$ na implementação) & $\sqrt{2\hat{\mu} T\, c_o^{\text{fix}}/h}$ & $c_o^{\text{fix}}{=}50$ instancia $K$; $h{=}1$; $\hat{\mu}$ estimado por série; $T$ é o horizonte de referência \\
EOQ & ponto de pedido $s$ ($s_{\text{ref}}$ na implementação) & $\hat{\mu} L + z\hat{\sigma}\sqrt{L}$ & $L{=}2$; $z{=}1{,}28$ para nível de serviço de referência de 90\%; ponto de pedido também chamado de ponto de reposição \\[4pt]

\multicolumn{4}{@{}l}{\textit{$(s,S)$}} \\[2pt]
$(s,S)$ & ponto de pedido $s$ & $\hat{\mu} L + z\hat{\sigma}\sqrt{L}$ (igual a $s_{\text{ref}}$ da EOQ) & $L{=}2$; $z{=}1{,}28$ \\
$(s,S)$ & nível alvo $S$ & $s + Q^*$ & $Q^*$ derivado da EOQ ($Q_{\text{ref}}$) \\
$(s,S)$ & fator de segurança $z$ & $1{,}28$ & quantil configurado para o nível de serviço de referência de 90\% \\[4pt]
...
GA & função de avaliação & $1{,}0 \times \mathrm{NS} - 0{,}0001 \times \mathrm{CTI}$ & aptidão escalar; rótulos finais usam menor CTI viável \\
GA & semente & 42 & \\[4pt]
...
Nos agentes de aprendizado por reforço, a decisão de reposição é formulada como um problema sequencial no ambiente de simulação. O estado observado pelo agente resume a posição de estoque, demanda prevista, pedidos em trânsito, estatísticas recentes de demanda e progresso no horizonte experimental. A ação representa a quantidade de pedido, discreta no caso do DQN e do SARSA e contínua no caso do PPO. A recompensa é definida como o negativo do custo operacional do ciclo, de modo que maximizar retorno equivale a reduzir custo total, penalizando simultaneamente estoque, ruptura e emissão de pedidos.
...
Ambiente & vetor de estado (dim.\ 1 a 3) & $I_t/I_0$,\; $\hat{D}/\mu$,\; $\mathrm{OP}/I_0$ & nível de estoque, previsão, pedido aberto \\
Ambiente & vetor de estado (dim.\ 4 a 6) & $\mu_6/\mu$,\; $\sigma_6/\sigma$,\; $t/T$ & tendências de curto prazo e posição no horizonte \\
Ambiente & função de recompensa & $-(h I_t + c_s S_t + K\mathbf{1}_{Q>0} + c_v Q_t)$ & $S_t$ = demanda não atendida \\[4pt]
...
POD / ADI $\times$ CV$^2$ & limiar ADI & 1,32 & classif.\ Syntetos-Boylan \\
POD / ADI $\times$ CV$^2$ & limiar CV$^2$ & 0,49 & classif.\ Syntetos-Boylan \\
POD / ADI $\times$ CV$^2$ & limiar de zeros consecutivos & 5 & \\
POD / ADI $\times$ CV$^2$ & limiar de burstiness & 0,30 & \\
POD / ADI $\times$ CV$^2$ & limiar ACF (sazonalidade) & 0,25 & \\
POD / ADI $\times$ CV$^2$ & coeficiente mínimo de tendência & 0,05 & \\
POD / ADI $\times$ CV$^2$ & limiar de entropia & 2,0 & \\
POD / ADI $\times$ CV$^2$ & média mínima de demanda & 2,0 & séries abaixo são excluídas \\
POD / ADI $\times$ CV$^2$ & limiar CV para segmentação & 1,5 & segmentação adicional por variabilidade \\[4pt]
...
% ─────────────────────────────────────────────────────────────────────────────
% TABELA A.6: Resultados agregados do Experimento 1 (Paraíba)
% ─────────────────────────────────────────────────────────────────────────────

\section{Resultados Agregados do Experimento 1}
\label{app:fase1_resultados}

Esta seção registra a tabela completa de indicadores agregados do Experimento~1 (15 lojas da Paraíba, produto~48130, regime \textit{Lumpy}), citada na Seção~\ref{sec:custo_politica_unica_vs_perfil} apenas em forma resumida para evitar duplicação com a comparação agregada do Experimento~2 (Tabela~\ref{tab:kpis_fase2}).
