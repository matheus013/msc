...
Este capítulo apresenta o arcabouço metodológico do AIPE, fundamentado na hipótese central de que o desempenho de políticas de reposição depende do regime operacional da demanda e que essa relação pode ser aprendida por um meta-modelo supervisionado. A exposição distingue a camada de entrada, composta por dados preparados, características operacionais e sinais auxiliares de previsão, do núcleo de seleção contextual responsável pela recomendação de políticas.

O AIPE é o \textit{framework} metodológico de seleção adaptativa; o PSE (\textit{Policy Selection Engine}) é o componente interno responsável por aprender a associação entre perfil de demanda e política recomendada. A ingestão, o pré-processamento, a extração de características operacionais e a previsão de demanda formam a camada de entrada que alimenta esse mecanismo de seleção. A partir desses insumos, o AIPE organiza o \textit{benchmark} das 12 políticas candidatas, a validação estatística formal, a geração de rótulos de política dominante e o treinamento do PSE, culminando na geração de relatórios e recomendações. A metodologia foi aplicada em duas fases: Fase~1 (concluída) e Fase~2, igualmente concluída, abrangendo 145 séries do estado da Bahia com validação estatística formal.
\section{Visão Geral do AIPE}
\label{sec:visao_geral_aipe}

Nesta dissertação, o AIPE é a proposta metodológica principal: um \textit{framework} de seleção contextual de políticas de reposição. O \textit{framework} recebe como entrada históricos loja-produto preparados, características operacionais extraídas das séries e, quando disponíveis, sinais auxiliares de previsão de demanda. A partir dessas entradas, o AIPE organiza a avaliação de políticas candidatas, gera rótulos de política dominante e treina o \textit{Policy Selection Engine} (PSE), responsável por recomendar a política mais adequada para novas séries. O PSE é, portanto, seu componente de seleção, e as políticas avaliadas compõem o portfólio de alternativas sobre o qual o \textit{framework} aprende a recomendar.

A preparação dos dados, a caracterização das séries e a previsão de demanda compõem a camada de entrada do processo. Essas etapas fornecem informações descritivas e preditivas para a seleção contextual, mas não constituem, isoladamente, a contribuição central da dissertação. O foco do AIPE está em transformar esses insumos em uma decisão de política de reposição, por meio de comparação empírica, validação estatística e aprendizado supervisionado.
...
A Figura~\ref{fig:aipe_architecture} apresenta a arquitetura conceitual do AIPE. A proposta distingue a camada de entrada, composta por dados preparados, características das séries, parâmetros operacionais e sinais auxiliares de previsão, do núcleo de seleção contextual, responsável por avaliar políticas candidatas, validar diferenças de desempenho, gerar rótulos e treinar o PSE. Essa separação permite que o \textit{framework} seja aplicado a outras bases, desde que a camada de entrada seja adaptada ao novo contexto operacional.
...
O problema considerado é um modelo de controle de inventário de único escalão com $N$ lojas varejistas atendidas por um armazém central. O horizonte de planejamento é discretizado em ciclos comerciais $t \in \{1, \ldots, T\}$, cada um com aproximadamente 21 dias corridos. As variáveis e parâmetros centrais são:
...
A Tabela~\ref{tab:dataset_stats} resume as principais características do \textit{dataset} nas duas fases experimentais. Essa síntese permite comparar o recorte controlado inicial com a ampliação posterior para múltiplas séries.
...
As características operacionais da demanda compõem a representação de entrada utilizada pelo AIPE. Elas descrevem o comportamento histórico da série loja-produto e permitem que o PSE aprenda associações entre perfis operacionais e políticas de reposição com melhor desempenho esperado.
...
A previsão de demanda é utilizada como fonte auxiliar de informação para a seleção contextual de políticas. Seus resultados não determinam isoladamente a política recomendada, mas fornecem sinais adicionais sobre o comportamento esperado da série, que podem ser incorporados à camada de entrada e ao treinamento do PSE. Quatro modelos (Croston, ARIMA/SARIMAX, XGBoost e LSTM) são treinados por série e selecionados por validação \textit{walk-forward} com métrica MASE \citep{Croston1972ForecastingIntermittentDemand}. A janela mínima de treinamento é $T_{\min} = 12$ ciclos; cada passo avança um ciclo, produzindo 20 a 26 pontos de teste. O protocolo elimina viés \textit{in-sample} e garante que previsões respeitem a ordem temporal dos dados. O modelo com menor MASE é selecionado como \textit{forecaster} oficial da série e suas métricas de acurácia (MASE, sMAPE, RMSSE) são incorporadas como sinais auxiliares de entrada. A previsão de demanda não é tratada como contribuição central nem como etapa final de decisão; o objetivo central permanece aprender qual política de reposição maximiza desempenho para cada regime operacional, não alcançar acurácia máxima em previsão.
...
\begin{itemize}
  ...
  \item Prazo de entrega fixo $L=2$ ciclos em ambas as fases experimentais;
...
A avaliação da Fase~2 segue um protocolo de validação temporal em janela deslizante \textit{(rolling horizon)} que integra os insumos experimentais e o núcleo de seleção contextual:
