...
Este capítulo apresenta o arcabouço metodológico do AIPE, fundamentado na hipótese central de que o desempenho de políticas de reposição depende do regime operacional da demanda e que essa relação pode ser aprendida por um meta-modelo supervisionado. A exposição conecta dados, simulação, validação estatística e seleção contextual em um protocolo único.
...
Esta seção formaliza o espaço de decisão, as métricas de avaliação e a estrutura de rede que constituem a base do \textit{benchmark} empírico. A definição desses elementos é necessária para garantir que todas as políticas sejam avaliadas sob condições comparáveis.
...
Uma \textit{política} $\pi$ mapeia o estado observado $(I_{t,i},\, D_{t-1,i},\, Q_{t-L,i})$ em uma decisão de pedido $Q_{t,i} = \pi(\cdot)$. O objetivo é determinar $\pi^*$ que minimize o custo total acumulado, considerando simultaneamente manutenção de estoque, ruptura e emissão de pedidos. A formulação adotada é:
...
\begin{itemize}
  ...
  \item Escalão 3 (Produto): conjunto de produtos do portfólio da rede. Na Fase~1, um único produto de alta intermitência para controle experimental; na Fase~2, extensão ao portfólio completo com filtragem por regime de demanda (Syntetos-Boylan).
\end{itemize}
...
Esta seção descreve o conjunto de dados transacional utilizado nos experimentos e os estágios de pré-processamento que transformam os registros brutos em séries prontas para simulação. A descrição também explicita as escolhas de filtragem que delimitam o escopo empírico das Fases~1 e~2.
...
A Tabela~\ref{tab:dataset_stats} sumariza as principais características do \textit{dataset} nas duas fases experimentais. Essa síntese permite comparar o recorte controlado inicial com a ampliação posterior para múltiplas séries.
...
Fase~1 aplica um recorte controlado: estado da Paraíba, produto único (48130, alta intermitência), lojas com $\geq 17$ ciclos com demanda positiva. Esse recorte foi usado para validar a dinâmica experimental em escala reduzida antes da ampliação do portfólio.

Fase~2 expande o escopo para o estado da Bahia e para múltiplos produtos ativos. A filtragem automática seleciona séries com CV $\geq 1.5$ e ciclos positivos suficientes para discriminar entre políticas. A extensão para todos os estados brasileiros compõe a Fase~3.
...
Os limiares são configuráveis via \texttt{demand\_profiling.yml} e calibráveis por análise exploratória do \textit{dataset}. Essa parametrização preserva a reprodutibilidade e permite adaptar o perfilamento a outros recortes de dados.
...
A Figura~\ref{fig:politica_ss} ilustra a lógica operacional da política $(s,S)$, usada como uma das referências clássicas no portfólio avaliado. A figura destaca o papel do ponto de reposição \(s\) e do nível-alvo \(S\), evidenciando que a decisão de pedido depende da posição observada do estoque no momento da revisão.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/política_de_reposição_de_estoques.pdf}
\caption{Representação conceitual da política $(s,S)$ no controle de inventário.}
\label{fig:politica_ss}
\end{figure}
...
Nas meta-heurísticas avaliadas, cada solução candidata representa uma política de reposição parametrizada pelo par $(s,Q)$, em que $s$ é o ponto de reposição e $Q$ é a quantidade de pedido. No Algoritmo Genético, esse par é tratado como o cromossomo da solução. A população inicial é gerada dentro dos limites operacionais definidos para cada série, e cada indivíduo é avaliado por simulação no ambiente de inventário. A seleção utiliza torneio, o cruzamento combina os valores dos pais para gerar novos pares $(s,Q)$, e a mutação aplica pequenas perturbações aos parâmetros, respeitando os limites viáveis. A função de aptidão combina nível de serviço e custo total de inventário, favorecendo soluções com maior atendimento da demanda e menor custo operacional. Os valores específicos de população, número de gerações, taxa de cruzamento, taxa de mutação e demais hiperparâmetros são apresentados no Apêndice~\ref{app:reproducibility}.

\textit{GA.} Seleção por torneio ($k=3$), cruzamento ($p_c=0.9$), mutação gaussiana; 100 indivíduos, 50 gerações. O parâmetro $k$ define o tamanho do torneio e $p_c$ a probabilidade de cruzamento.

A Figura~\ref{fig:ga_cromossomo} ilustra a representação adotada para as soluções candidatas no Algoritmo Genético. Cada indivíduo codifica parâmetros de uma política de reposição, avaliados por simulação no ambiente de inventário, e essa representação permite aplicar operadores de seleção, cruzamento e mutação sobre soluções comparáveis.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/representação_do_cromossomo_no_ag.pdf}
\caption{Representação do cromossomo no Algoritmo Genético para parametrização de políticas de reposição.}
\label{fig:ga_cromossomo}
\end{figure}

\textit{SA.} Temperatura inicial $T_0=1000$, resfriamento $\alpha=0.95$; aceitação probabilística $\exp(-\Delta C / T_k)$, em que $\Delta C$ é a variação de custo e $T_k$ é a temperatura na iteração $k$. Esses parâmetros definem a exploração inicial e sua redução progressiva ao longo da busca.

\textit{PSO.} 40 partículas, 80 iterações; inércia $w=0.7$, acelerações $c_1=c_2=1.5$, em que $w$ controla persistência da velocidade e $c_1$, $c_2$ controlam atração cognitiva e social. Cada partícula corresponde a uma parametrização candidata da política avaliada.

\textit{DE.} Evolução Diferencial \textit{rand/1/bin}; fator de mutação $F=0.8$, taxa de cruzamento $C_R=0.9$, em que $F$ controla a amplitude da mutação diferencial e $C_R$ a probabilidade de cruzamento. Representações visuais complementares do PSO e da Evolução Diferencial são apresentadas no Apêndice~\ref{ap:representacoes_politicas}.
...
A Figura~\ref{fig:dqn} resume a forma como um agente DQN utiliza o estado observado do ambiente para estimar valores associados a ações candidatas de reposição. Essa representação ilustra a lógica geral dos agentes de aprendizado por reforço considerados no portfólio, nos quais decisões de pedido são avaliadas a partir de recompensas observadas no ambiente de simulação.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/representação_do_agente_dqn.pdf}
...
A Figura~\ref{fig:ga_dqn} sintetiza a lógica de combinação entre busca evolutiva e refinamento por aprendizado por reforço. Essa política híbrida é tratada nesta dissertação como uma especialista do portfólio avaliado, e não como a contribuição central do trabalho, que permanece concentrada na seleção contextual realizada pelo AIPE.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/representação_da_política_híbrida_ga_dqn.pdf}
\caption{Representação da política híbrida GA-DQN no portfólio avaliado.}
\label{fig:ga_dqn}
\end{figure}

A política GA-PPO segue a mesma motivação geral de combinar exploração evolutiva e refinamento por aprendizado, mas utiliza o PPO como mecanismo de atualização estável da política. Sua representação visual complementar é apresentada no Apêndice~\ref{ap:representacoes_politicas}.

\section{Simulação de Inventário}

O ambiente \textit{InventoryEnv} simula o modelo de inventário $(s,Q)$ descrito na Seção~\ref{sec:formulacao}, sobre o qual todas as doze políticas são avaliadas em condições idênticas e controláveis. Essa padronização evita que diferenças de desempenho sejam atribuídas a ambientes de teste distintos.
...
Todos os testes são realizados ao nível de significância $\alpha = 0.05$ com correção de Bonferroni para comparações múltiplas. Esse protocolo reduz o risco de interpretações baseadas em diferenças ocasionais entre políticas.

\section{Seleção Contextual de Políticas}

O \textit{Policy Selection Engine} (PSE) mapeia o perfil operacional de cada série à política de menor custo total de inventário esperado entre as que atendem ao limiar mínimo de nível de serviço, aprendendo a partir do \textit{benchmark} das Fases~1 e~2. Essa etapa transforma os resultados simulados em uma regra de recomendação para séries não vistas.

\subsection{Base Conceitual}

O \textit{Policy Selection Engine} (PSE) é uma instância de \textit{meta-aprendizado} aplicado a decisões operacionais: aprende, a partir da experiência acumulada no \textit{benchmark} (``quais políticas funcionaram bem em séries com estas características?''), a mapear o perfil de uma nova série para a política de menor custo total de inventário esperado. Essa formulação aproxima o problema de reposição de uma tarefa de seleção supervisionada de políticas.
...
Esta subseção delimita o escopo experimental adotado para avaliação preliminar do AIPE. Diferentemente da seção de escopo geral apresentada na Introdução, o foco aqui está nas escolhas operacionais do experimento: fases avaliadas, subconjuntos de dados, políticas comparadas, métricas, replicações e premissas de simulação. Com isso, a seção descreve como a avaliação foi conduzida, sem repetir as delimitações conceituais da pesquisa.

O protocolo experimental distingue duas fases avaliadas:
...
Em ambas as fases, as políticas candidatas são avaliadas no mesmo ambiente de simulação e comparadas por custo total de inventário, nível de serviço, ruptura, efeito chicote e frequência de pedidos. As premissas de horizonte temporal, \textit{lead time}, custos, estoque inicial, sementes aleatórias e hiperparâmetros das políticas permanecem fixadas conforme a matriz experimental do Apêndice~\ref{app:reproducibility}, enquanto a ampliação para outros produtos, lojas, estados e regimes de demanda é tratada como continuidade da Fase~3.
