...
com $z = 1.28$, quantil normal associado ao nível de serviço de referência de 90\%. Nas Equações~\ref{eq:I0_metod} a~\ref{eq:sref_metod}, $I_0$ é o estoque inicial usado na simulação, $Q_{\text{ref}}$ é o lote de referência derivado da lógica EOQ, $s_{\text{ref}}$ é o ponto de reposição de referência, $\hat{\mu}$ e $\hat{\sigma}$ são a média e o desvio-padrão estimados na janela histórica disponível, $T$ é o horizonte experimental, $L$ é o prazo de entrega, $c_o^{\text{fix}}$ é o custo fixo por pedido e $h$ é o custo de manutenção. Essa parametrização estabelece uma escala operacional comum para o \textit{benchmark}, sem determinar antecipadamente a política vencedora.
