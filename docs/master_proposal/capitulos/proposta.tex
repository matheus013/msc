...
\section{Formulação do Problema de Otimização de Inventário}
...
com $z = 1.28$ (nível de serviço alvo de 90\%). Os espaços de busca das meta-heurísticas e os limites de ação dos agentes de RL são dimensionados proporcionalmente a $Q_{\text{ref}}$ e $s_{\text{ref}}$, garantindo comparação justa entre lojas com volumes heterogêneos (razão de até 124 vezes entre a maior e a menor loja na Fase~1). Na Fase~2, $\hat{\mu}$ e $\hat{\sigma}$ são atualizados a cada ciclo com a janela deslizante, implementando calibração \textit{online}.
...
Esta seção descreve o portfólio de doze políticas candidatas organizadas em quatro famílias de crescente sofisticação computacional. Todas as políticas são avaliadas no mesmo ambiente \textit{InventoryEnv} com as mesmas séries de demanda real, garantindo comparação controlada.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/política_de_reposição_de_estoques.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/representação_do_cromossomo_no_ag.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/representação_do_agente_dqn.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{img/representação_da_política_híbrida_ga_dqn.pdf}
...
\section{Simulação e Benchmark de Políticas}
\label{sec:simulacao_benchmark}

O ambiente \textit{InventoryEnv} serve como plataforma de benchmark comum para todas as doze políticas candidatas, implementando o modelo de inventário $(s,Q)$ descrito na Seção~\ref{sec:formulacao}. A padronização das condições de avaliação garante que diferenças de desempenho sejam atribuídas ao comportamento das políticas, não ao ambiente de teste.
...
\section{Seleção Contextual e Treinamento do PSE}
...
Nesta dissertação, a seleção de políticas é formulada como uma instância supervisionada do problema de seleção de algoritmos (\textit{Algorithm Selection Problem}, ASP), no qual cada série loja-produto é tratada como uma instância descrita por atributos operacionais. O método não busca uma política universal para toda a rede, mas aprende, a partir dos resultados simulados, qual política do portfólio apresenta melhor desempenho para cada tipo de série. A política dominante é a de menor custo total de inventário entre as políticas que atendem ao limiar mínimo de nível de serviço, tornando a seleção orientada simultaneamente por eficiência de custo e viabilidade operacional.

Assim, a taxonomia de Syntetos-Boylan e as demais características operacionais caracterizam a série; o \textit{benchmark} simulado identifica a política dominante segundo CTI e NS; e o PSE aprende a relação entre perfil operacional e política recomendada.
...
O PSE não se limita a uma classificação supervisionada de algoritmos. É um mecanismo metodológico de seleção contextual: aprende, a partir da experiência acumulada no \textit{benchmark}, quais estruturas de demanda favorecem quais famílias de política. Sua saída não é um rótulo de algoritmo, mas uma recomendação acionável com probabilidades de confiança, o que permite ao gestor compreender a margem de decisão e selecionar alternativas quando a política recomendada não está disponível.
...
\section{Aplicação, Recomendações e Reavaliação Periódica}
